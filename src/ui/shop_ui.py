...
class ShopUI:
    def __init__(self, items: dict[str, Item]):
        ...
        for obj in ui_layer.tiled_objects:
            ...
            if obj.name == "text_box":
                ...
                self._box_amount_text = self._make_label("", x, y, w, h)
                self._box_amount_widget.add(self._box_amount_text)

            elif obj.name == "amount_text":
                self._amount_text = self._make_label("", x, y, w, h)
                self._amount_widget.add(self._amount_text)

            elif obj.name == "text":
                self._text = self._make_label("", x, y, w, h)
        ...
        self._option_labels = []
        self._build_option_labels(["BUY", "SELL", "EXIT"])
    ...
    def _build_item_labels(self):
        for i, item in enumerate(self._item_names):
            ...
            label = self._make_label(
                ...
                self._item_size["x"],
                self._item_size["y"] - i * self._item_size["h"],
                self._item_size["w"],
                self._item_size["h"],
            )
    ...
    def _build_option_labels(self, options: list[str]):
        for i, option in enumerate(options):
            label = self._make_label(
                option,
                self._option_size["x"],
                self._option_size["y"] - i * self._option_size["h"],
                self._option_size["w"],
                self._option_size["h"],
            )
    ...
    def _make_label(self, text: str, x, y, w, h) -> arcade.gui.UILabel:
    ...
    def _update_cursor(self, index: int, use_options: bool = False):
        ...
        self._cursor.x = label.rect.left - 25
