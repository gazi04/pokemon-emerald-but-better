...
class EnemyAI:
    ...
    def simulate_damage(
        self, attacker: BattlePokemon, defender: BattlePokemon, move: PokemonMove
    ) -> int:
        if not move.power:
            return 0
        ...
        for effect in move.effects:
            ...
            if effect.type == EffectType.STAT:
                ...
                if (
                    ...
                    if status_condition == StatusEffect.BURN and "fire" in target.types:
                        continue
                    if status_condition == StatusEffect.POISON and (
                        "poison" in target.types or "steel" in target.types
                    ):
                        continue
                    if status_condition == StatusEffect.PARALYSIS and (
                        "ground" in target.types or "electric" in target.types
                    ):
                        continue
    ...
    def simulate_turn(
        ...
        is_player_first = self._player_moves_first(
            player_pokemon.get_stat(Stat.SPEED),
            player_move.priority,
            enemy_pokemon.get_stat(Stat.SPEED),
            enemy_move.priority,
        )

        if is_player_first:
            enemy_pokemon.current_hp -= self.simulate_damage(
                player_pokemon, enemy_pokemon, player_move
            )
            points -= self.simulate_effects_move(
                player_move, player_pokemon, enemy_pokemon
            )

            if enemy_pokemon.current_hp > 0:
                player_pokemon.current_hp -= self.simulate_damage(
                    enemy_pokemon, player_pokemon, enemy_move
                )
                points += self.simulate_effects_move(
                    enemy_move, enemy_pokemon, player_pokemon
                )
        else:
            player_pokemon.current_hp -= self.simulate_damage(
                enemy_pokemon, player_pokemon, enemy_move
    ...
    def _evaluate_ai_move(
        ...
        for p_move in player_moves:
            ...
            if (
                depth <= 1
                or enemy_pokemon_copy.current_hp <= 0
                or player_pokemon_copy.current_hp <= 0
            ):
                outcomes.append(
                    turn_score
                    + self.evaluate_hp(enemy_pokemon_copy, player_pokemon_copy)
                )
            else:
                future_score = max(
                    self._evaluate_ai_move(
                        enemy_pokemon_copy, player_pokemon_copy, next_move, depth - 1
                    )
    ...
    def select_move(
        ...
        best_move_indices = sorted(
            move_scores.keys(), key=lambda idx: move_scores[idx], reverse=True
        )
