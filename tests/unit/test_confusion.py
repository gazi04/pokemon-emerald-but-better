...
def test_confusion_coexists_with_major_status():
    ...
    effect = PokemonMoveEffect(
        ...
        condition=StatusEffect.CONFUSION,
        chance=100,
    )
    move = PokemonMove(
        "confuse ray", "status", "ghost", 0, 100, 10, 0, 0, None, None, [effect]
    )
    attacker = _mon()
...
def test_confused_pokemon_can_hurt_itself():
    ...
    with patch(
        "src.model.battle.battle_pokemon.random.random", return_value=0.0
    ):  # <1/3 -> self-hit
...
def test_confusion_wears_off():
    ...
    with patch(
        "src.model.battle.battle_pokemon.random.random", return_value=0.99
    ):  # no self-hit
        messages, can_move = mon.check_can_move(0)
